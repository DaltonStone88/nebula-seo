...
export default function ComparisonPage({ competitor = '', tagline = '', intro = '', rows = [], verdict = '', ourWins = [], theirWins = [] }) {
  return (
    <>
      ...
      <Navbar />
      <style>{`
        .comp-row:hover { background: rgba(232,238,255,0.03) !important; }
        @media(max-width:768px){
          .comp-hero{padding:120px 24px 60px!important}
          .comp-section{padding:0 24px 60px!important}
          .comp-wins{grid-template-columns:1fr!important}
          .comp-table-header{grid-template-columns:1.5fr 1fr 1fr!important}
          .comp-table-row{grid-template-columns:1.5fr 1fr 1fr!important}
          .comp-verdict-cta{flex-direction:column!important;text-align:center}
          .comp-other{flex-wrap:wrap}
          nav{padding:16px 20px!important}
          nav ul{display:none!important}
        }
      `}</style>

      <section className="comp-hero" style={{ padding: '160px 60px 80px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 16, color: 'var(--dim)', lineHeight: 1.75, marginBottom: 48, maxWidth: 600, margin: '0 auto 48px' }}>{tagline}</p>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 32, marginBottom: 60, flexWrap: 'wrap' }}>
      ...
      <section className="comp-section" style={{ padding: '0 60px 60px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <p style={{ fontSize: 15, color: 'var(--dim)', lineHeight: 1.85 }}>{intro}</p>
        </div>
      </section>

      <section className="comp-section" style={{ padding: '0 60px 80px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div className="comp-wins" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      ...
      <section className="comp-section" style={{ padding: '0 60px 80px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div className="comp-table-header" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', borderRadius: '12px 12px 0 0', overflow: 'hidden', border: '1px solid var(--border)' }}>
          ...
          {rows.map((row, i) => (
            <div key={i} className={`comp-row comp-table-row`} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', border: '1px solid var(--border)', borderTop: 'none', background: i % 2 === 0 ? 'transparent' : 'rgba(232,238,255,0.01)', transition: 'background 0.15s' }}>
              <div style={{ padding: '13px 20px', fontSize: 13, color: 'var(--dim)', borderRight: '1px solid var(--border)' }}>{row.feature}</div>
              <div style={{ padding: '13px 20px', textAlign: 'center', borderRight: '1px solid var(--border)', fontSize: 13, color: row.us && row.us.startsWith('✓') ? 'rgba(20,200,100,0.9)' : row.us && row.us.startsWith('✗') ? 'rgba(255,80,80,0.7)' : 'var(--star-white)' }}>
                {row.us}
              </div>
              <div style={{ padding: '13px 20px', textAlign: 'center', fontSize: 13, color: row.them && row.them.startsWith('✓') ? 'rgba(20,200,100,0.7)' : row.them && row.them.startsWith('✗') ? 'rgba(255,80,80,0.5)' : 'var(--dim)' }}>
                {row.them}
              </div>
            </div>
          ))}
      ...
      <section className="comp-section" style={{ padding: '0 60px 80px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <div className="comp-verdict-cta" style={{ padding: '36px', borderRadius: 18, background: 'linear-gradient(135deg, rgba(123,47,255,0.12), rgba(0,200,255,0.06))', border: '1px solid rgba(123,47,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 }}>
            <div>
            ...
            <Link href="/login?signup=true" className="btn-primary" style={{ padding: '14px 36px', fontSize: 12, flexShrink: 0 }}>Start $1 Trial →</Link>
          </div>
        </div>
      </section>

      <section className="comp-section" style={{ padding: '0 60px 100px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ fontSize: 13, color: 'var(--dim)', marginBottom: 16 }}>Other comparisons</div>
          <div className="comp-other" style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {['Paige', 'LocalViking', 'BrightLocal', 'Yext'].filter(c => c !== competitor).map(c => (
              <Link key={c} href={`/compare/nebulaseo-vs-${c.toLowerCase()}`} style={{ padding: '8px 20px', borderRadius: 20, border: '1px solid var(--border)', fontSize: 13, color: 'var(--dim)', transition: 'all 0.2s' }}
